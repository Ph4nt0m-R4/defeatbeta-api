...
class Configuration:
    ...
    def get_duckdb_settings(self):
        ...
        backend = self.get_http_cache_backend()
        if backend == 'cache_httpfs':
            settings = [
    ...
    def get_http_cache_backend(self):
        if self.http_cache_backend in ('cache_httpfs', 'httpcachefs'):
            return self.http_cache_backend
