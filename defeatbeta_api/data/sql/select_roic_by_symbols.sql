...
base_data_window AS (
  ...
  FROM
      ...
      JOIN (
          ...
          FROM
              grouped_data
          WHERE (symbol, continuous_id) IN (
              SELECT symbol, MAX(continuous_id) FROM grouped_data GROUP BY symbol
          )
      ) t2
  ON t1.group_id = t2.group_id AND t1.symbol = t2.symbol
  ORDER BY
      symbol, continuous_id ASC
),
...
select symbol,
  ...
  from invested_capital_avg order by symbol, report_date;
